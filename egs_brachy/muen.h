 ...
 *
 * The muen data is either in the format generated by
 * the egsnrc usercode `g`, or by the muendat-generation.py tool:\verbatim
 
    #
    # Original formatting
    #
    Muen values for medium MEDIUM = MED_NAME_1
    Medium used is MED_NAME_1 found in path/to/material.dat
    ...
    Medium used is MED_NAME_2 found in path/to/material.dat
    Number of energy intervals is 500
    Energy[MeV] Muen[cm^2/g]
    0.001000 4075.692785 
    0.001004 4036.185761 
    0.001007 3997.307705 
    (# 497 more lines)
 \endverbatim
 */
    ...
    MuenMapT splitFileByMed(ifstream &in) {
        ...
        while (in) {
            ...
            if (trim(line).size() == 0 || line[0] == '#') {
                // allow blank lines
                // allow comments if they start at the beginning of the line
                continue;
            }
            ...
            if (new_med) {
                ...
                try{
                    switch (components.size())  {
                        case 4:
                            // old format
                            energy = stod(components[0]);
                            muen   = stod(components[2]);
                            break;
                        case 2:
                            // new format
                            energy = stod(components[0]);
                            muen   = stod(components[1]);
                            break;
                        default:
                            err_flag = true;
                    }
                } catch (const invalid_argument&) {
                    // this is thrown by stod if you try to convert something that isn't a double
                    err_flag = true;
                } 
